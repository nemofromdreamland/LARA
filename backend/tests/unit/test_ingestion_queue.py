...
Uses the autouse fakeredis fixture from conftest (backs session_store._get_redis)
...
from app.services.session_store import _get_redis, get_job_status
...
async def test_stale_message_is_reclaimed_and_completed():
    ...
    assert data["status"] == "done"
    assert (await _get_redis().xpending(iq._STREAM, iq._GROUP))["pending"] == 0
...
async def test_ensure_consumer_group_is_idempotent():
    ...
    groups = await _get_redis().xinfo_groups(iq._STREAM)
